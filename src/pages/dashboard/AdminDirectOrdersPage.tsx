...
const AdminDirectOrdersPage = () => {
  ...
  const filteredOrders = directOrders
    .filter((order) => {
  ...
  const deliveredCount = directOrders.filter(o => o.order_status === 'delivered').length;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Direct Store Orders</h1>
          <p className="text-muted-foreground">Orders for products you own directly</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-emerald-600">{formatPKR(directRevenue)}</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">Total Orders</p>
            <p className="text-2xl font-bold">{directOrders.length}</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
      ...
      {/* Orders Table */}
      <Card>
        <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Store className="h-5 w-5" /> Direct Orders ({filteredOrders.length})</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="h-32">
                        <div className="flex flex-col items-center justify-center text-center py-6">
                          ...
                          <p className="font-medium text-foreground">No direct store orders found</p>
                          <p className="text-sm text-muted-foreground mt-1">Orders for your own products will appear here.</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
...
export default AdminDirectOrdersPage;
